...
- El orquestador vive en `factory/run.ts` y `factory/orchestrator.ts`; los hooks en `.claude/hooks/`. La cola de tareas está en `factory/queue.yaml` y se consume con `run.ts --next`, que es lo que llama launchd.
